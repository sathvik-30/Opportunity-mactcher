...
def compute_match_score(student_skills: list, opportunity: dict) -> int:
    ...
    try:
        ...
        # Raised by TfidfVectorizer when both documents are effectively
        # empty (e.g. a student with no skills listed) — no meaningful
        # overlap to score, so 0 is correct here. Anything else is an
        # unexpected bug and should surface, not be silently swallowed.
        return 0
...
def check_eligibility(student: dict, opportunity: dict) -> dict:
    ...
    if min_year and student["year"] < min_year:
        issues.append(f"Requires {_ordinal(min_year)} year or above")
    else:
